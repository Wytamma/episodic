...
Maximum clade credibility (MCC) trees are generated for each clock and duplicate run, with node heights summarised according to the `heights` option (for example, `mean`, `median`, `keep`, `ca`). Here we can see the elevated foreground rate on the stem branches leading into the VOC clades in the FLC shared-stem model, which is also somewhat captured by the relaxed clock.
...
<div style="display:flex; gap:12px; align-items:flex-start;">
  ...
  <div style="flex:1; text-align:center;">
    <p>Relaxed</p>
    <img src="/images/relaxed_0.5_0.1_2.mcc.mean.svg" alt="Relaxed clock MCC tree" style="width:100%;" />
  </div>
  <div style="flex:1; text-align:center;">
    <p>Strict</p>
    <img src="/images/strict_0.5_0.1_2.mcc.mean.svg" alt="Strict clock MCC tree" style="width:100%;" />
  </div>
</div>